...
# https://github.com/havakv/pycox/issues/162. Incompatibility between pycox and pandas' new version (assert that you are using a right pandas version, such as 1.5.3)
import warnings
...
class SAVAE(VariationalAutoencoder):
    ...
    def predict_time(self, x, device=torch.device('cpu')):
        ...
        time_params = self.Time_Decoder(torch.from_numpy(out_params['z']).float())

        # Sample covariate and time values
        cov_params = out_params['cov_params']
